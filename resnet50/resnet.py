...
def run_benchmark(model, args):
    ...
    train_acc_factor = None
    for factor in tracking_factors:
        if factor.name == '%s_%s_train_acc' % (args.data_set, args.batch_size):
            train_acc_factor = factor
    train_speed_factor = None
    for factor in tracking_factors:
        if factor.name == '%s_%s_train_speed' % (args.data_set, args.batch_size):
            train_speed_factor = factor
...
if __name__ == '__main__':
    ...
    if args.device == 'GPU':
        collect_memory_thread = threading.Thread(target=collect_gpu_memory_data)
